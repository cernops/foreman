 ...
 def multiple_checkboxes(f, attr, klass, associations, options = {}, html_options={})
    if associations.count > 5
      field(f, attr,options) do
        ...
        hidden_fields + f.collection_select(attr_ids, associations.all.sort_by { |a| a.to_s }  , :id, :to_s ,options.merge(:selected => selected_ids), html_options.merge(:multiple => true))
      end
    else
      field(f, attr, options) do
  ...
  def get_hostgroup_parent(hostgroup, name='')
