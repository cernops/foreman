...
class Host < Puppet::Rails::Host
  ...
  validate :ensure_owner_type, :if => Proc.new { |host| host.owner_type.present? }     
  ...
  def ensure_owner_type
    errors.add(:owner_type, "Must be User or Usergroup") unless %w(User Usergroup).include?(owner_type)
  end
